...
function DialogOverlay({
  ...
  return (
    <DialogPrimitive.Backdrop
      ...
      className={cn(
        // Materialize's modal overlay is a flat, heavy black — the blur and the
        // 10% scrim were doing the separation job the opacity should do.
...
function DialogFooter({
  ...
  return (
    <div
      ...
      className={cn(
        // --secondary for the same reason CardFooter takes it: --muted does not
        // read against a white surface.
        "-mx-4 -mb-4 flex flex-col-reverse gap-2 rounded-b-md border-t bg-secondary p-4 sm:flex-row sm:justify-end",
        className
      )}
      {...props}
    >
...
function DialogDescription({
  ...
  return (
    <DialogPrimitive.Description
      ...
      className={cn(
        "text-sm text-muted-foreground *:[a]:text-msg-link *:[a]:hover:text-foreground",
        className
      )}
      {...props}
    />
