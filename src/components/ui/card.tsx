...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        // --card and --background are both white, so the #e0e0e0 border is the
        // only thing making this read as a box. The old translucent ring was
        // derived from --foreground and is far too faint to carry that alone.
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "font-heading text-[15px] leading-snug group-data-[size=sm]/card:text-sm",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        // --secondary, not --muted: the footer sits on a white card, where
        // #fafafa is a 1.04:1 step and reads as nothing at all.
        "flex items-center rounded-b-md border-t bg-secondary p-(--card-spacing)",
        className
      )}
      {...props}
    />
